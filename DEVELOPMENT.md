...
| `src/scorer/realized_returns.py` | Realized-return persistence: `compute_realized_return_for_row(conn, ticker, signal_date, forward_days, benchmark)` computes the actual N-day forward return and SPY excess return for a single row; `populate_realized_returns(conn, *, force, dry_run, batch_size)` iterates eligible `scores_daily` rows and writes 5 columns (`realized_trading_days`, `realized_ticker_return`, `benchmark_return`, `realized_excess`, `realized_computed_at`). Reads `forward_days` from `config/scorer.json["analytics"]["forward_days"]` (NOT `calibration.forward_days`). A WARNING is logged per invocation when the two keys diverge. Called as a non-fatal sub-step from `scripts/run_daily.py` after the scorer phase; also invoked by `scripts/backfill_realized_returns.py` for historical population. |
| `src/scorer/confidence.py` | Signal classification; confidence modifiers; `data_completeness`; `key_signals`. The cold-start confidence base is config-driven via `confidence.cold_start_base_multiplier` in `scorer.json` (default 0.3, currently 0.65) — see `src/scorer/main.py` inline multiplier resolution at the `confidence_base_score` assignment. Modifying the multiplier requires `--historical --force` re-run of the scorer. |
| `src/scorer/contribution.py` | Builds the per-indicator/per-pattern/per-aggregate contribution payload; called by `score_ticker` after `apply_adaptive_weights` for the daily payload (with `aggregate_scores`) and again after the weekly breakdown for the weekly payload (with `aggregate_scores={}`). Persisted as JSON in `scores_daily.key_signals_data` (daily) and `scores_weekly.key_signals_data` (weekly). |
...
| `src/notifier/ai_reasoner.py` | `reason_all_qualifying_tickers()` — Claude API calls per qualifying ticker. `get_qualifying_tickers()` filters out cold-start tickers (`calibrated_score IS NULL`) from both bullish and bearish alert candidates regardless of their confidence value; cold-start tickers with `calibrated_score IS NULL` are never included in Telegram alerts. |
...
| `web/src/lib/scoring/categoryMap.ts` | `INDICATOR_CATEGORY_MAP` + `INDICATOR_DISPLAY_LABELS` (12 entries) — mirrors Python's `category_scorer.py`; drift-guarded by `tests/web/test_category_map_sync.py` |
| `web/src/lib/scoring/patternLabels.ts` | `PATTERN_DISPLAY_LABELS` map + `humanizePatternName()` utility — converts snake_case DB pattern names to human-readable labels for the matrix pattern rows |
...
- `scores_weekly` — composite PRIMARY KEY `(ticker, week_start)`. Stores the weekly composite, regime, eight category scores, `data_completeness`, a JSON `key_signals` array, and a nullable `key_signals_data` TEXT column holding the per-indicator contribution payload (indicator + pattern items; no aggregate items).
