...
## Current Task
**Sitting 2 — wire `/why` into Telegram (`/why` command handler, `CallbackQueryHandler`, inline button on `/detail` msg #3, atomic deploy).**
...
| 2026-04-29 | **Daily card enriched with `key_signals` top-3, earnings row, and signal-flip badge.** 46 new tests, 115 total in `tests/web/`. Docs: CONFIG.md, DESIGN.md §14.2+§14.6, DEVELOPMENT.md. | ✅ Done |
...
- **Sitting 2 — `/why` Telegram wiring**: register `CommandHandler("why", ...)` and `CallbackQueryHandler` in `src/notifier/bot.py`; add inline button to `/detail` msg #3; atomic deploy via `deploy.sh`.
