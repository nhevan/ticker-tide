...
Click-to-expand panel beneath each indicator row in the matrix. Clicking a row toggles the panel; clicking again or clicking a different row replaces/collapses it.

### 15.1 Contract

The server is the authoritative source for all numbers. The frontend is a pure renderer. No client-side scoring or recalculation is performed.

### 15.2 RSI 7-step trace

For `indicator === "rsi_14"` the panel renders a faithful 7-step trace:

1. **Raw value** — `snapshot.daily.indicators.rsi_14`. The true RSI reading (0–100). **Not** `contributions_payload.items[*].raw_value` (which stores the indicator score −100…+100, not the raw measurement).
2. **Zone label** — `snapshot.daily.rsi_zone_label`. Generated by `zone_label_for_rsi()` in `src/scorer/zone_labels.py`. Profile path yields six labels (`extreme_oversold`, `oversold`, `below_mid`, `above_mid`, `overbought`, `extreme_overbought`); fallback path yields four.
3. **Scoring path** — presence/absence of `snapshot.daily.rsi_profile`. Profile path lists p5/p20/p50/p80/p95; fallback path lists the fixed thresholds from `/api/scoring-rules`.
4. **Indicator score** — `snapshot.daily.indicator_scores.rsi_14` (−100 to +100).
5. **Magnitude share in momentum** — `abs(rsi_score) / sum(abs(score) for all momentum items)` from the contributions payload.
6. **Category weight × expansion** — `item.category_weight` and `payload.expansion_factor` from the contributions payload (not from the rules endpoint — uses the value active at scoring time for historical consistency).
7. **Net contribution** — `item.contribution` verbatim from the contributions payload.

### 15.3 Divergence caveat

Item-level contributions do not sum to the final composite score because:
- Category scores are clamped to ±100 before weighting.
- A sector adjustment is applied after the category-weighted sum.
- Three timeframes (daily, weekly, monthly) are merged with regime-adaptive weights.

The `approximation_caveat` string from `/api/scoring-rules` is shown in step 7.

### 15.4 `/api/scoring-rules` endpoint

Process-static endpoint. Returns:
- `rsi.thresholds` — oversold/overbought from `config/scorer.json`.
